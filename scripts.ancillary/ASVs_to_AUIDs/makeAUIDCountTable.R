...
##
## For each ASV count table create an AUID count table by substituting in the
## AUID for the ASV id.  The ASV-to-AUID mapping is within the table returned by
## assignAUIDs2ASVs.R. The table also includes a Source column for the ASV
...
Options:
    ...
    -merge  Rather than separate files, produce one table with all abundances,
            AUID_abundances.tsv.
"
...
if (mergeTabs) {
    ...
    stopifnot(names(which(table(unlist(sapply(x, colnames))) != 1)) == 'label')
    m <- x[[1]]
    if (length(x) > 1) {
        for (i in 2:length(x)) {
            m <- merge(m, x[[i]], by='label', all=T)
        }
        ## merge() used NA when an AUID was absent in a table.
        m[is.na(m)] <- 0
    }
    ## Verify ASV abundance preserved.
    stopifnot(sum(sapply(allTabs, function(x) sum(x[,colnames(x)!='label']))) == sum(m[,-1]))
    ## For consistency with the separate count tables, make the row names be the AUIDs (or md5s)
    stopifnot(length(unique(m$label)) == nrow(m))
    rownames(m) <- m$label
    m <- m[,colnames(m) != 'label']
    m <- m[order(rowSums(m), decreasing=T),]
    cat("There are", nrow(m), "AUIDs and", ncol(m), "samples.\n")
    write.table(m, file="AUID_abundances.tsv", sep="\t", row.names=T, quote=F)
}
